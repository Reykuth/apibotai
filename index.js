...
let isCreatingCode = false; // เพิ่มตัวแปรเพื่อติดตามสถานะการสร้างโค้ด
...
async function handleEvent(event) {
  ...
  if (event.message.text === '/สร้างโค้ด') {
    isCreatingCode = true; // ตั้งค่าสถานะเป็น true เมื่อผู้ใช้พิมพ์ /สร้างโค้ด
    return client.replyMessage(event.replyToken, {
      type: 'text',
      text: 'กรุณาตั้งชื่อโค้ดของคุณ'
    });
  } else if (isCreatingCode) { // ตรวจสอบสถานะก่อนสร้างโค้ด
    isCreatingCode = false; // รีเซ็ตสถานะ
